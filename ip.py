...
for start in range(1,11):
    ...
    url = 'https://www.kuaidaili.com/free/inha/{}/'.format(start)       # 每页12个数据，共爬取10页
    print("正在处理url: ",url)
    ...
    ip = soup.select('#list > div.table-section > table > tbody > tr > td:nth-child(1)')
    port = soup.select('#list > div.table-section > table > tbody > tr > td:nth-child(2)')
...
def check_ip(list_ip):
    ...
    for ip in list_ip:
        try:
            response = requests.get(url=url,headers=headers,proxies=ip,timeout=1)      # 在1秒之内请求百度的服务器
